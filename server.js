...
function getSpinStatusForUser(userId) {
  ...
  const inWindow = isSpinWindowOpen(settings);
  return {
    ...
    can_spin: inWindow && spinsUsedToday < dailyLimit
  };
}
...
async function route(req, res) {
  ...
  if (req.method === 'POST' && url.pathname === '/spin/play') {
    ...
    if (!status.can_spin) return json(res, 400, { error: 'Spin not available right now' });
